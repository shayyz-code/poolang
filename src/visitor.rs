...
impl ScopedSymbolTable {
    ...
    pub fn get_field_or_method(&self, object_name: &str, field_or_method: &str) -> Option<Type> {
        if let Some(Type::Object(fields)) = self.get(object_name) {
            return fields.get(field_or_method).cloned();
        }
        None
    }
}
